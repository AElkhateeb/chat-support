...
/**
 * Class FillDefaultSupportAdminAndPermissions
 */
class FillDefaultSupportAdminAndPermissions extends Migration
{
    /**
    ...
    /**
     * FillDefaultSupportAdminAndPermissions constructor.
     */
    public function __construct()
    {
        $this->guardName = config('support-auth.defaults.guard');
        ...
        $defaultPermissions = collect([
            ...
            // manage translations
            'support.translation.index',
            'support.translation.edit',
            'support.translation.rescan',

            // manage users (access)
            'support.support-admin.index',
            'support.support-admin.create',
            'support.support-admin.edit',
            'support.support-admin.delete',

            // ability to upload
            'support.upload',

            //ability to impersonal login
            'support.support-admin.impersonal-login'
        ]);
        ...
        $this->roles = [
            [
                ...
                'permissions' => $defaultPermissions->reject(function ($permission) {
                    return $permission === 'support.support-admin.impersonal-login';
                }),
            ],
        ];
    ...
    public function up(): void
    {
        if ($this->userClassName === null) {
            throw new RuntimeException('Support user model not defined');
        }
        DB::transaction(function () {
            ...
            foreach ($this->users as $user) {
                ...
                if ($userItem === null) {
                    ...
                    SupportAdmin::find($userId)->addMedia(storage_path() . '/images/avatar.png')
                        ->preservingOriginal()
    ...
    public function down(): void
    {
        ...
        DB::transaction(function () {
            foreach ($this->users as $user) {
                ...
                if ($userItem !== null) {
                    SupportAdmin::find($userItem->id)->media()->delete();
