...
class MessagePageController extends Controller
{
    ...
    public function index(IndexChat $request)
    {
        ...
        // pass the request with params
            ...
            function ($query) use ($request) {
                $query->with('messages', function ($q) {
                    if(isset($_GET['chat'])){
                        $q->with('sender')->with('customer')->where('chat_id',$_GET['chat'])->latest();
                    }else{
                        $q->with('sender')->with('customer')->where('chat_id',1)->latest();
                    }

                })->with('sender')->with('customer')->orderBy('created_at', 'DESC');
            }
        );
      ...
      // return ['data' => $data];
       return view('admin.chat', ['data' => $data]);
    }
    ...
    {
        $this->authorize('admin.chat.create');
    ...
    public function store(StoreChat $request)
    {
        ...
        if ($request->ajax()) {
            return ['redirect' => url('admin/chats'), 'message' => trans('brackets/admin-ui::admin.operation.succeeded')];
        }
    ...
    public function store(StoreMessage $request)
    {
        ...
        $sanitized['media'] ='NO';
        $sanitized['sender_type'] ='Brackets\AdminAuth\Models\AdminUser';
        ...
        $eventNewMessage=[
            ...
            'sender_type' => 'Brackets\AdminAuth\Models\AdminUser',
...
/*
        if ($request->ajax()) {
            return ['redirect' => url('admin/messages?chat='.$request->get('chat_id')), 'message' => trans('brackets/admin-ui::admin.operation.succeeded')];
        }

        return redirect('admin/messages?chat='.$request->get('chat_id'));*/
    ...
    {
        $this->authorize('admin.chat.show', $chat);
        ...
        // pass the request with params
            ...
            // set columns to query
            ['id', 'client_type', 'client_id', 'sender_type', 'sender_id','body'],

            // set columns to searchIn
            ['id', 'client_type', 'sender_type', 'body'],
            function ($query) use ($request) {
                $query->with('messages', function ($q) {
                    if(isset($_GET['chat'])){
                        $q->with('sender')->with('client')->where('chat_id',$_GET['chat']);
                    }else{
                        $q->with('sender')->with('client')->where('chat_id',1);
                    }

                })->with('sender')->with('client')->where('chat_id',1);
            }
        );
    ...
    public function show(IndexMessage $request)
    {
       // return $request;
        ...
        // pass the request with params
            ...
            function ($query) use ($request) {
                $query->latest()->take(5)->with('sender')->with('customer')->where('chat_id',$request->chat);
            }
        );

       // $messages= array_reverse(array($data),true);
    ...
    {
        $this->authorize('admin.chat.edit', $chat);
