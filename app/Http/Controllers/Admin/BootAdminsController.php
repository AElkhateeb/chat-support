...
class BootAdminsController extends Controller
{
    ...
    public function index(IndexBootAdmin $request)
    {
        ...
        return view('admin.boot-admin.index', ['data' => $data]);
    }
    ...
    {
        $this->authorize('admin.boot-admin.create');

        return view('admin.boot-admin.create', [
    ...
    {
        $this->authorize('admin.boot-admin.show', $bootAdmin);
    ...
    {
        $this->authorize('admin.boot-admin.edit', $bootAdmin);

        $bootAdmin->load('previous');
        return view('admin.boot-admin.edit', [
            'bootAdmin' => $bootAdmin,
            'patterns' => BootAdmin::where('stuff', '!=' ,'Identify')->get(),
        ]);
    }
    ...
    public function update(UpdateBootAdmin $request, BootAdmin $bootAdmin)
    {
        ...
        if ($request->ajax()) {
            return [
                'redirect' => url('admin/boot-admins'),
