...
use App\Http\Requests\Ceo\AdminUser\DestroyAdminUser;
use App\Http\Requests\Ceo\AdminUser\ImpersonalLoginAdminUser;
use App\Http\Requests\Ceo\AdminUser\IndexAdminUser;
use App\Http\Requests\Ceo\AdminUser\StoreAdminUser;
use App\Http\Requests\Ceo\AdminUser\UpdateAdminUser;
...
class AdminUsersController extends Controller
{
    ...
    public function index(IndexAdminUser $request)
    {
        ...
        return view('ceo.admin-user.index', ['data' => $data, 'activation' => Config::get('ceo-auth.activation_enabled')]);
    }
    ...
    {
        $this->authorize('ceo.admin-user.create');

        return view('ceo.admin-user.create', [
            'activation' => Config::get('ceo-auth.activation_enabled'),
            'roles' => Role::where('guard_name','ceo')->get(),
        ]);
    }
    ...
    public function store(StoreAdminUser $request)
    {
        ...
        if ($request->ajax()) {
            return ['redirect' => url('ceo/admin-users'), 'message' => trans('brackets/admin-ui::admin.operation.succeeded')];
        }

        return redirect('ceo/admin-users');
    }
     ...
     * @return void
     */
    public function show(CeoAdmin $adminUser)
    {
        $this->authorize('ceo.admin-user.show', $adminUser);
     ...
     * @return Factory|View
     */
    public function edit( CeoAdmin $adminUser)
    {
        $this->authorize('ceo.admin-user.edit', $adminUser);

        $adminUser->load('roles');

        return view('ceo.admin-user.edit', [
            'ceoUser' => $adminUser,
            'activation' => Config::get('ceo-auth.activation_enabled'),
            'roles' => Role::where('guard_name','ceo')->get(),
        ]);
