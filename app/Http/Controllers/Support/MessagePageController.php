...
class MessagePageController extends Controller
{
    ...
    public function index(IndexChat $request)
    {
       ...
       //return ['data' => $data];
       return view('support.chat', ['data' => $data]);
    }
    ...
    public function store(StoreMessage $request)
    {
        ...
        $eventNewMessage=[
            ...
            'img' => ($data[0]['customer_type']=='App\Models\Customer')? 'https://ptetutorials.com/images/user-profile.png' : Auth::user()->avatar_thumb_url,
...
/*
        if ($request->ajax()) {
            return ['redirect' => url('support/messages?chat='.$request->get('chat_id')), 'message' => trans('brackets/admin-ui::admin.operation.succeeded')];
        }

        return redirect('support/messages?chat='.$request->get('chat_id'));*/
    ...
    public function edit(Chat $chat)
    {
       ...
       // return view('support.chat.edit', [ 'chat' => $chat,]);
    }
