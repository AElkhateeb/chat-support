...
class SupportAdminsController extends Controller
{
    ...
    public function index(IndexAdminUser $request)
    {
        ...
        return view('support.support-admin.index', ['data' => $data, 'activation' => Config::get('support-auth.activation_enabled')]);
    }
    ...
    {
        $this->authorize('support.support-admin.create');

        return view('support.support-admin.create', [
    ...
    public function store(StoreAdminUser $request)
    {
        // Sanitize input
         ...
         $sanitized['department'] =$request->get('department')['id'];
        // Store the AdminUser
        ...
        if ($request->ajax()) {
            return ['redirect' => url('support/support-admin'), 'message' => trans('brackets/admin-ui::admin.operation.succeeded')];
        }

        return redirect('support/support-admin');
    }
    ...
    {
        $this->authorize('support.support-admin.show', $adminUser);
    ...
    {
        $this->authorize('support.support-admin.edit', $adminUser);

        $adminUser->load('roles');

        return view('support.support-admin.edit', [
            'adminUser' => $adminUser,
            'activation' => Config::get('support-auth.activation_enabled'),
            'roles' => Role::where('guard_name','support')->get(),
        ]);
