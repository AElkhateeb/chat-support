...
class BootAdminsController extends Controller
{
    ...
    public function index(IndexBootAdmin $request)
    {
       ...
       // return ['data' => $data];
        return view('ceo.boot-admin.index', ['data' => $data]);
    }
    ...
    {
        $this->authorize('ceo.boot-admin.create');

        return view('ceo.boot-admin.create', [
    ...
    {
        $this->authorize('ceo.boot-admin.show', $bootAdmin);
    ...
    {
        $this->authorize('ceo.boot-admin.edit', $bootAdmin);

        $bootAdmin->load('previous');
        
        return view('ceo.boot-admin.edit', [
            'bootAdmin' => $bootAdmin,
            'patterns' => BootAdmin::where('stuff', '!=' ,'Identify')->get(),
        ]);
    }
    ...
    public function update(UpdateBootAdmin $request, BootAdmin $bootAdmin)
    {
        ...
        if ($request->ajax()) {
            return [
                'redirect' => url('ceo/boot-admins'),
                'message' => trans('brackets/admin-ui::admin.operation.succeeded'),
            ];
        }

        return redirect('ceo/boot-admins');
    }
