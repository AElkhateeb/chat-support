...
namespace App\Http\Controllers\Ceo\Auth;
...
class LoginController extends Controller
{
    ...
    */
     ...
     * @var string
     */
    protected $redirectToAfterLogout = '/ceo/login';
    ...
    public function __construct()
    {
        ...
        $this->redirectTo = config('ceo-auth.login_redirect');
        $this->redirectToAfterLogout = config('ceo-auth.logout_redirect');
       $this->middleware('guest.admin:' . $this->guard)->except('logout');
       // $this->middleware('guest.ceo:' . $this->guard)->except('logout');
    }
    ...
    {
        $conditions = [];
        if (config('ceo-auth.check_forbidden')) {
            $conditions['forbidden'] = false;
        }
        if (config('ceo-auth.activation_enabled')) {
