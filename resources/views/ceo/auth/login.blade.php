	        ...
	        <div class="col-md-6 col-lg-5">
				<div class="card">
					<div class="card-block">
						<auth-form
								:action="'{{ url('/ceo/login') }}'"
								:data="{}"
								inline-template>
							<form class="form-horizontal" role="form" method="POST" action="{{ url('/ceo/login') }}" novalidate>
								{{ csrf_field() }}
								...
								<div class="auth-body">
									@include('ceo.auth.includes.messages')
									...
									<div class="form-group text-center">
										<a href="{{ url('/ceo/password-reset') }}" class="auth-ghost-link">{{ trans('brackets/admin-auth::admin.login.forgot_password') }}</a>
									</div>
								</div>
							</form>
						</auth-form>
