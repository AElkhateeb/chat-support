...
<?php $department=array(
    array('id'=>'technical','label'=>'Technical'),
    array('id'=>'sales','label'=>'Sales'),
) ?>
...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('department'), 'has-success': fields.department && fields.department.valid }">
    <label for="department" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-3'">{{ trans('admin.admin-user.columns.department') }}</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-7'">
        <multiselect v-model="form.department" placeholder="{{ trans('brackets/admin-ui::admin.forms.select_an_option') }}" label="label" track-by="id":options="{{json_encode($department)}}" open-direction="bottom"></multiselect>
