...
<div class="form-check row" :class="{'has-danger': errors.has('opened'), 'has-success': fields.opened && fields.opened.valid }">
    <div class="ml-md-auto" :class="isFormLocalized ? 'col-md-8' : 'col-md-10'">
        <input class="form-check-input" id="opened" type="checkbox" v-model="form.opened" v-validate="''" data-vv-name="opened"  name="opened_fake_element">
        <label class="form-check-label" for="opened">
            {{ trans('admin.customer.columns.opened') }}
        </label>
        <input type="hidden" name="opened" :value="form.opened">
        <div v-if="errors.has('opened')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('opened') }}</div>
    </div>
</div>
